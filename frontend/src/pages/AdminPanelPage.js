...
import { Activity, Database, FileLock2, LockKeyhole, MonitorCog, ShieldCheck, Sparkles, UserCog, Users } from "lucide-react";
  ...
  const renderOverview = () => (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
        <MetricTile dataTestid="admin-metric-patients" detail={t("common.active")} label={t("admin.patients")} value={overview?.summary?.totalPatients || 0} />
        <MetricTile dataTestid="admin-metric-doctors" detail={t("common.active")} label={t("admin.doctors")} value={overview?.summary?.activeDoctors || 0} />
        <MetricTile dataTestid="admin-metric-caregivers" detail={t("common.active")} label={t("admin.caregivers")} value={overview?.summary?.activeCaregivers || 0} />
        <MetricTile dataTestid="admin-metric-pending-consents" detail={t("common.consents")} label={t("admin.openConsents")} value={overview?.summary?.pendingConsentApprovals || 0} />
        <MetricTile dataTestid="admin-metric-alerts" detail={t("common.active")} label={t("admin.activeAlerts")} value={overview?.summary?.activeAlerts || 0} />
        <MetricTile dataTestid="admin-metric-system" detail={t("common.systemHealth")} label={t("common.systemHealth")} value={overview?.summary?.aiSystemStatus || "healthy"} tone="glow" />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
          ...
          <GlassPanel className="space-y-4" dataTestid="admin-ai-model-status-panel" variant="strong">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div className="grid gap-3">
                <RiskCard dataTestid="admin-ai-runtime-card" icon={Sparkles} label={t("admin.aiRuntime")} sublabel={t("common.systemHealth")} tone="mint" value="Online" />
                <RiskCard dataTestid="admin-storage-card" icon={Database} label={t("admin.storage")} sublabel="Mongo" tone="cyan" value={overview?.summary?.storageStatus || "stable"} />
              </div>
            </div>
          </GlassPanel>
        </div>
        ...
        <div className="space-y-4">
          <AiStatusRing dataTestid="admin-consent-overview-ring" label={t("admin.consentOverview")} score={`${consentRate}%`} status="Granted" subScores={[{ label: "Audio", value: "92%" }, { label: "Video", value: "78%" }, { label: "Location", value: "88%" }, { label: "Reports", value: "81%" }]} />
          <GlassPanel className="space-y-4" dataTestid="admin-quick-actions-panel" variant="strong">
            <p className="font-display text-2xl text-white">{t("admin.privacyGdpr")}</p>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
        <GlassPanel className="space-y-4" dataTestid="admin-audit-log-table" variant="strong">
          <p className="font-display text-2xl text-white">{t("admin.latestAuditLogs")}</p>
          {(overview?.auditLogs || []).slice(0, 6).map((entry) => (
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4" key={entry.id || `${entry.action}-${entry.timestamp}`}>
              ...
              <p className="mt-2 text-sm text-white/58">{entry.target} · {entry.consentStatus || "—"} · {formatDateTime(entry.timestamp, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
            </div>
          ))}
        </GlassPanel>
        <GlassPanel className="space-y-4" dataTestid="admin-access-control-panel" variant="strong">
          <p className="font-display text-2xl text-white">{t("admin.accessControl")}</p>
          {[
            { icon: UserCog, title: t("admin.permissions"), value: `${overview?.summary?.activeDoctors || 0}/${overview?.summary?.activeCaregivers || 0}` },
            { icon: LockKeyhole, title: t("common.consents"), value: `${consentRate}%` },
            { icon: FileLock2, title: t("common.audit"), value: `${overview?.auditLogs?.length || 0}` },
          ].map((item) => (
            <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4" key={item.title}>
              ...
              <span className="font-mono-display text-lg text-white">{item.value}</span>
            </div>
          ))}
  ...
  const renderUsers = () => (
    <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
      <GlassPanel className="space-y-4" dataTestid="admin-users-table" variant="strong">
        <p className="font-display text-2xl text-white">{t("admin.patients")}</p>
      ...
      <GlassPanel className="space-y-4" dataTestid="admin-caregivers-table" variant="strong">
        <p className="font-display text-2xl text-white">{t("admin.permissions")}</p>
